...
export class AuthService {
  ...
  async signUp(signUpData: SignUpRequest): Promise<AuthResult> {
    try {
      ...
      // Better-Auth manages sessions internally via tokens
  ...
  async signOut(sessionToken: string): Promise<void> {
    try {
      ...
      // Additional cleanup: remove session from our repository
  ...
  async validateSession(sessionToken: string): Promise<SessionValidationResult> {
    try {
      ...
      // Validate with Better-Auth first
      ...
      // Better-Auth validates the token internally, we represent the session state
      const sessionInfo = this.createSessionFromToken(sessionToken, result.user);
  ...
  async refreshSession(sessionToken: string): Promise<AuthResult | null> {
    try {
      ...
      // Refresh session with Better-Auth
      const result = await auth.api.getSession({
        headers: new Headers({
          'authorization': `Bearer ${sessionToken}`,
        }),
      ...
      // Better-Auth validates the token, we represent the current session state
      const sessionInfo = this.createSessionFromToken(sessionToken, result.user);
   ...
   * Better-Auth handles sessions internally via JWT tokens, so we create
   * a session object that represents the token-based session state
   */
