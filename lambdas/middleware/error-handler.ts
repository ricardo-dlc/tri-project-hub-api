...
import { MiddlewareOptions, ProcessingResult, ErrorMapping } from './types';
...
export const classifyError = (error: Error): 'http' | 'custom' | 'unknown' => {
  // Check if it's a known HttpError using instanceof
  if (error instanceof HttpError) {
    return 'http';
  }
  
  // Check if it's a custom error with statusCode property using optional chaining
...
export const sanitizeError = (error: Error, isProduction = false): Error => {
...
export const shouldRetryError = (error: Error, attempt: number, maxRetries = 3): boolean => {
